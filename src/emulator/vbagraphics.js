class VbaGraphics {
  constructor(isGba, emscriptenModule, canvas, gbBorder) {
    this.isGba = isGba;
    this.GBA_WIDTH = isGba ? 240 : gbBorder ? 258 : 162;
    this.GBA_HEIGHT = isGba ? 160 : gbBorder ? 225 : 145;
  ...
  setGbBorderOn() {
    this.display = false;
    this.clearImageData(this.image, this.imageData, this.pixelCount, true);
    this.GBA_WIDTH = 258;
    this.GBA_HEIGHT = 225;
    this.pixelCount = this.GBA_WIDTH * this.GBA_HEIGHT;
    this.initScreen();
    setTimeout(() => {
      this.clearImageData(this.image, this.imageData, this.pixelCount, false);
      this.setGbBorderClass();
      this.display = true;
    }, 100);
  }
